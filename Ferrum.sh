...
#-! Defines what type of Download is used and if its a valid or an exit code
    DL_TYPE="$(jq -r '.code' ~/FERRUM/Config/DownloadType.json)"
    DL_INDEX="$(jq -r '.index' ~/FERRUM/Config/DownloadType.json)"

    if [[ $DL_TYPE -ne "-1" ]] 
        then
          termux-notification -t "FERRUM OPERATION CANCELED" -c "FERRUM encoutered an error or the operation was cancelled by the user"
    else
        case $DL_INDEX in
            ...
            2) 
                termux-dialog radio -t "FERRUM Options" -v "Output format,Lyrics Provider,Path Template,Output Folder" > ~/FERRUM/Config/OptionHandler.json 
                    ...
                    OPTION_CODE="$(jq -r '.code' ~/FERRUM/Config/OptionHandler.json)"
                    if [ $OPTION_CODE -ne "-1" ]
                        then termux-notification -t "FERRUM OPERATION CANCELED" -c "FERRUM encoutered an error or the operation was cancelled by the user"
                        else
